...
class Magister {
  ...
  appointments() {
    // extract options
    const _ref = _lodash.default.find(arguments, _lodash.default.isPlainObject) || {},
          _ref$fillPersons = _ref.fillPersons,
          fillPersons = _ref$fillPersons === void 0 ? false : _ref$fillPersons,
          _ref$fetchAbsences = _ref.fetchAbsences,
          fetchAbsences = _ref$fetchAbsences === void 0 ? true : _ref$fetchAbsences,
          _ref$ignoreAbsenceErr = _ref.ignoreAbsenceErrors,
          ignoreAbsenceErrors = _ref$ignoreAbsenceErr === void 0 ? true : _ref$ignoreAbsenceErr; // extract dates
  ...
  createAppointment(options) {
    ...
    const payload = {
      ...
      Inhoud: function () {
        const content = _lodash.default.trim(options.content);

        return content.length > 0 ? _lodash.default.escape(content) : null;
      }(),
      Type: options.type || 1,
  ...
  fileFolders() {
    return this._privileges.needs('bronnen', 'read').then(() => this.http.get(`${this._personUrl}/bronnen?soort=0`)).then(res => res.json()).then(res => res.Items.map(f => new _fileFolder.default(this, f)));
  }
  /**
    ...
    return _asyncToGenerator(function* () {
      ...
      function () {
        var _ref2 = _asyncToGenerator(function* () {
          const accountData = yield self.http.get(`${schoolUrl}/api/account`).then(res => res.json());
          const id = accountData.Persoon.Id; // REVIEW: do we want to make profileInfo a function?

          self.profileInfo = new _profileInfo.default(self, accountData.Persoon);
          self._privileges = new _privileges.default(self, accountData.Groep[0].Privileges);
