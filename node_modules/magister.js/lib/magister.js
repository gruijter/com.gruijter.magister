  ...
  */
	children() {
		...
		return this.http.get(`${this._personUrl}/kinderen`).then(res => res.json()).then(res => res.Items).then(items => items.map(raw => {
			...
			m.profileInfo = new _profileInfo2.default(this, raw);
	...
	login(forceLogin = false) {
		...
		return _asyncToGenerator(function* () {
			...
			try {
				...
				throw new _authError2.default(err);
			}

			// extract bearer token
			const token = yield _this.http.get(`https://accounts.magister.net${returnUrl}`, {
				redirect: 'manual',
				headers: {
			...
			const accountData = yield _this.http.get(`${schoolUrl}/api/account`).then(function (res) {
				return res.json();
			});
			const id = accountData.Persoon.Id;

			// REVIEW: do we want to make profileInfo a function?
			_this.profileInfo = new _profileInfo2.default(_this, accountData.Persoon);
			_this._privileges = new _privileges2.default(_this, accountData.Groep[0].Privileges);

			_this._personUrl = `${schoolUrl}/api/personen/${id}`;
			_this._pupilUrl = `${schoolUrl}/api/leerlingen/${id}`;
 ...
 * @readonly
 */
const VERSION = exports.VERSION = '2.0.0-alpha.0';
