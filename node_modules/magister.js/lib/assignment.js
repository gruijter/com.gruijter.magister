...
class Assignment extends _magisterThing2.default {
  ...
  */
   ...
   * @readonly
   */
		this.files = raw.Bijlagen.map(f => new _file2.default(magister, undefined, f));
		/**
   * @type Person[]
   * @readonly
   */
		this.teachers = raw.Docenten != null ? raw.Docenten.map(p => new _person2.default(magister, p)) : [];
		/**
   * @type String[]
   * @readonly
   */
  ...
  */
	versions() {
		const promises = this.versionIds.map(id => {
			...
			return this._magister.http.get(url).then(res => res.json()).then(raw => new _assignmentVersion2.default(this._magister, raw));
		});
