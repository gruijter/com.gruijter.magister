...
class Appointment extends _magisterThing.default {
  /**
   * @private
  ...
  constructor(magister, raw) {
    ...
    this.description = _lodash.default.trim(raw.Omschrijving);
    /**
     * @type {String}
     * @readonly
     */

    this.location = _lodash.default.trim(raw.Lokatie);
    /**
     * @type {String}
     * @readonly
     */
    ...
    this.classes = raw.Vakken != null ? _lodash.default.map(raw.Vakken, 'Naam') : []; // REVIEW: moeten we de key 'Naam' wel plucken?
    ...
    this.teachers = raw.Docenten != null ? _lodash.default.map(raw.Docenten, p => new _person.default(magister, p)) : [];
    /**
     * @type {String[]}
     * @readonly
     */

    this.classRooms = raw.Lokalen != null ? _lodash.default.map(raw.Lokalen, 'Naam') : []; // REVIEW: moeten we de key 'Naam' wel plucken?
  ...
  get status() {
    switch (this._status) {
      ...
      // Geroosterd automatisch

      case 2:
        return 'scheduled manually';
      // Geroosterd handmatig
  ...
  attachments() {
    ...
    return this._magister.http.get(this._url).then(res => res.json()).then(res => {
      const attachments = res.Bijlagen;
      const person = this.teachers[0];
      return attachments.map(raw => {
        const f = new _file.default(this._magister, undefined, raw);
        f.addedBy = person;
        return f;
      });
    });
  ...
  remove() {
    ...
    return this._magister._privileges.needs('afspraken', 'delete').then(() => this._magister.http.delete(this._url));
  }
  /**
  ...
  saveChanges() {
    return this._magister._privileges.needs('afspraken', 'update').then(() => this._magister.http.put(this._url, this._toMagister())).then(() => undefined);
  }
  /**
   * @private
   * @returns {Object}
