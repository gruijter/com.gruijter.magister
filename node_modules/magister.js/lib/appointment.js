...
class Appointment extends _magisterThing2.default {
  ...
  */
   ...
   * @readonly
   */
		this.teachers = raw.Docenten != null ? _lodash2.default.map(raw.Docenten, p => new _person2.default(magister, p)) : [];
		/**
   * @type String[]
   * @readonly
   */
	...
	attachments() {
		...
		return this._magister.http.get(this._url).then(res => res.json()).then(res => {
			...
			return attachments.map(raw => {
				const f = new _file2.default(this._magister, undefined, raw);
				f.addedBy = person;
				return f;
			});
		});
