...
class Message extends _magisterThing2.default {
  ...
  */
   ...
   */
		...
		if (raw != null) {
			...
			this.body = undefined;
			this.recipients = raw.Ontvangers.map(p => new _person2.default(magister, p));
  ...
  */
	addRecipient(recipients) {
		...
		if (!recipients.every(x => x instanceof _person2.default)) {
  ...
  */
	fill(fillPersons = false) {
		...
		return this._magister.http.get(url).then(res => res.json()).then(res => {
			...
			this.attachments = (res.Bijlagen || []).map(a => new _file2.default(this._magister, undefined, a));

			if (fillPersons) {
				let promises = [];

				// fill sender
				promises.push(this.sender.getFilled().then(r => this.sender = r).catch(() => this.sender));
