...
class MessageFolder extends _magisterThing.default {
  /**
   * @private
  ...
  messages({
    ...
    if (count === 0) {
      ...
      url += '&count=true';
      return this._magister._privileges.needs('berichten', 'read').then(() => this._magister.http.get(url)).then(res => res.json()).then(res => res.TotalCount).then(totalCount => {
        count = Math.min(count, totalCount);

        const promises = _lodash.default.chain(count / MAX_LIMIT).range().map(n => this.messages({
          count: MAX_LIMIT,
          skip: n * MAX_LIMIT,
          readState,
          fill,
        ...
        return Promise.all(promises).then(objects => ({
          messages: (0, _lodash.default)(objects).map('messages').flatten().value(),
          totalCount
        }));
      });
    }

    return this._magister._privileges.needs('berichten', 'read').then(() => this._magister.http.get(url)).then(res => res.json()).then(res => {
      const messages = res.Items.map(m => new _message.default(this._magister, m));
      const promise = fill ? Promise.all(messages.map(m => m.fill(fillPersons))) : Promise.resolve(messages);
  ...
  toJSON() {
    return _lodash.default.omit(super.toJSON(), 'type');
  }

}

var _default = MessageFolder;
